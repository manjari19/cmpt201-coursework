...
static void initialize_block(struct header *b, uint64_t sz, struct header *n,
                             int id) {
  b->size = sz;
  b->next = n;
  b->id = id;
...
static int find_worst_fit(struct header *free_list_ptr, uint64_t need) {
  ...
  uint64_t worst_sz = 0;
  for (struct header *p = free_list_ptr; p; p = p->next)
    if (p->size >= need && p->size > worst_sz) {
      worst_sz = p->size;
      worst_id = p->id;
    }
  return worst_id;
...
Assume free list is kept in address order. Idea: walk once, merge neighbors that
touch.
...
After free(z):
  ...
  keep going until no more joins. Reduces fragmentation, makes bigger blocks
possible.
*/
